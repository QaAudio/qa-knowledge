...
const KEYWORD_PAYLOAD_FIELDS = ["skill_name", "source_type"];
/** Create the collection when missing, then ensure the filterable payload indexes. */
export async function ensureCollection(client, collection, dimensions) {
 ...
 */
export async function ensurePayloadIndexes(client, collection) {
...
export async function upsertChunks(client, collection, chunks, vectors) {
...
export async function deleteChunksByIds(client, collection, ids) {
    if (ids.length === 0)
        return 0;
...
export async function searchChunks(client, collection, vector, options = {}) {
...
function toSearchHit(hit) {
    ...
    return {
        ...
        source_type: String(payload.source_type ?? "repo_doc"),
...
export function matchesFilters(hit, options) {
...
export async function getChunkById(client, collection, chunkId) {
