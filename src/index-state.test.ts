...
import { type CorpusFingerprint, diffCorpus, embeddingCompatible } from "./index-state.js";

function fp(contentHash: string, metaHash = "m"): CorpusFingerprint {
...
test("diffCorpus: classifies unchanged, changed, added, removed", () => {
  ...
  const current = new Map<string, CorpusFingerprint>([
    ["a.md", fp("1")], // unchanged
...
test("diffCorpus: meta-only change marks file changed", () => {
  ...
  const current = new Map<string, CorpusFingerprint>([["a.md", fp("1", "m2")]]);
  const diff = diffCorpus(current, state, true);
...
test("diffCorpus: no prior state treats everything as added", () => {
  const current = new Map<string, CorpusFingerprint>([["a.md", fp("1")], ["b.md", fp("2")]]);
...
test("embeddingCompatible: detects dimension/model drift", () => {
  const state = stateWith({});
  assert.equal(
    embeddingCompatible(state, { provider: "openrouter", model: "m", dimensions: 1536 }),
    true,
  );
  assert.equal(
    embeddingCompatible(state, { provider: "openrouter", model: "m", dimensions: 768 }),
    false,
  );
  assert.equal(
    embeddingCompatible(state, { provider: "ollama", model: "m", dimensions: 1536 }),
    false,
  );
});
