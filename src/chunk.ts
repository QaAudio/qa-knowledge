...
export function chunkDocument(input: ChunkInput): KnowledgeChunk[] {
...
type Section = { title: string; headingPath?: string; body: string };

function splitMarkdown(text: string): Section[] {
  ...
  for (const line of lines) {
    ...
    if (h1 || h2 || h3) {
      ...
      if (h1) currentPath = [label];
      else if (h2) currentPath = [currentPath[0] ?? "", label].filter(Boolean);
      else if (h3) currentPath = [...currentPath.slice(0, 2), label].filter(Boolean);
      currentTitle = label;
      buffer.push(line);
      continue;
    }
...
function splitSdkDts(text: string): Section[] {
  const sections: Section[] = [];
...
function splitWithOverlap(text: string): string[] {
  ...
  while (start < text.length) {
    ...
    if (end < text.length) {
      ...
      if (breakAt > start + MIN_CHUNK_CHARS) end = breakAt;
    }
