# Guardrails — scan, additive-first, don't clobber

## Scan before you change
Always `ableton_scan_context` (and `ableton_scan_track` for arrangement clips,
`ableton_read_clip_notes` / `ableton_read_device` / `ableton_read_drum_rack_map` for detail)
before mutating. You need to know: which tracks/slots are occupied, the key &
tempo, what the user already made. Re-scan after structural edits (handles are
ephemeral, indices shift).

**One failed scan/read tool does not mean abandon scan/read tools** — fix arguments (e.g. wrap
`addr`, use a device addr for Drum Rack) and retry; do not read clips/notes via `run_code`
when scan/read tools still apply.

## Additive-first
Prefer creating **new** tracks/clips/devices over editing existing ones:
- New track: `ableton_run_code` with `await song.createMidiTrack()` / `createAudioTrack()`, then write to its empty `clipSlots[0]` (MIDI) or add audio via `run_code`.
...
- **Only read-only scans run without a confirm:** `scan_context` / `read_device` /
  `read_clip_notes` / `read_selection`, and `run_code` that only returns data.
