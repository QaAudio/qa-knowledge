...
Live and returns a small summary. Adjust key/tempo to the Set (`ableton_scan_context`).
...
```ts
const song = context.application.song;
const clip = song.tracks[0].clipSlots[0].clip;   // resolve the target via scan_context's addr in practice
