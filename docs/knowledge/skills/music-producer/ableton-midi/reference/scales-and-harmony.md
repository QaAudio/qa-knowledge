...
`ableton_scan_context` returns the Set's scale:
```jsonc
...
const { rootNote: root, scaleIntervals: iv } = { rootNote: 9, scaleIntervals: [0,2,3,5,7,8,10] }; // A minor; or read from scan_context
