...
`Balance` −1..1). **Read `min`/`max` from `read_device` and scale accordingly** — e.g.
...
1. Find the device `addr` from `ableton_scan_context` (`devices[].addr`), or insert one.
2. `ableton_read_device {addr}` → full param list with current values, ranges, value labels.
...
const track = song.tracks[0];                       // resolve via scan_context addr in practice
