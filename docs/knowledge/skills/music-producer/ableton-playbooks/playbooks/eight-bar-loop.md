...
| Param | Default | Notes |
|-------|---------|------|
| key | the Set's `scale` (`rootNote`/`scaleIntervals`), else A minor | Scan first; coherence |
...
## Procedure
1. **Scan** (`ableton_scan_context`): read `tempo`, `scale`, existing tracks.
...
5. **Verify**: `ableton_read_clip_notes` on the chord/bass/lead clips → notes in key;
