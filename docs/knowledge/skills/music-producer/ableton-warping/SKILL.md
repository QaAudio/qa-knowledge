...
description: Work with audio clips, warping, and tempo in Ableton Live via the qa-ableton-mcp MCP tools — enable/set warp mode on audio clips, read warp markers, and reason about tempo. Use when handling audio (not MIDI) clips or time-stretching. Lower priority for the MIDI-centric v1.
---
...
- Audio clips expose `warping`, `warpMode` (get/set), `warpMarkers`, `filePath`
  (`search_knowledge` for the Ableton SDK types → AudioClip, WarpMode enum).
...
- Read warp info via `ableton_read_clip_notes` on the audio clip's `addr` (returns
