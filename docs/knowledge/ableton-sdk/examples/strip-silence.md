...
export function activate(activation: ableton.ActivationContext) {
  ...
  context.commands.registerCommand("example.stripSilence", (arg: unknown) =>
    void (async (selection: ableton.ArrangementSelection) => {
      ...
      await context.ui.withinProgressDialog("Strip Silence", {}, async (update, abortSignal) => {
        try {
          ...
          const results: { track: ableton.AudioTrack<"1.0.0">; silence: SilenceRange[] }[] = [];
    ...
    })(arg as ableton.ArrangementSelection).catch((e) => console.error(e)),
  );
